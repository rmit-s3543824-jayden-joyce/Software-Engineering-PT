...
public class BusinessManagementTest {
	...
	// Test if the business is not found in the business.txt, an IOException will be thrown
	@Test
	...
	// Test if selectBusiness is able to retrieve the chosen business from the business.txt
	@Test
	...
	// Test if retreiveBooking is able to obtain bookings from the text file and set up an arrayList.
	@Test
	...
	public void testViewNewBookings() throws IOException{
		...
		// After bookings are viewed, there are no longer bookings with the status "New"
		assertEquals(false,thereAreNewBookings);
		
	}
	
	// To test if the input is being validated correctly, only allowing positive integers as the input
	@Test
